...
type
  ...
  private
    ...
    procedure setFAreaSquareFeet(const Value: Double);
  ...
  public
     ...
     property AreaSquareFeet     : Double    read FAreaSquareFeet     write setFAreaSquareFeet;
  ...
  public
    ...
    procedure Save;
    procedure Update;
    procedure Search(varId_sup_invoice : Integer);
...
procedure TInvoice.Search(varId_sup_invoice: Integer);
var
 sqlDados : TFDQuery;
begin
  with DBDados do
  begin
    ...
    Try
      ...
      sqlDados.Close;
      sqlDados.SQL.Clear;
      sqlDados.SQL.Add('SELECT ID_SUP_INVOICE ');
      sqlDados.SQL.Add(',ID_SUPPLIER ');
      sqlDados.SQL.Add(',ID_COMPANY ');
      sqlDados.SQL.Add(',DATE_INVOICE ');
      sqlDados.SQL.Add(',INVOICE_ID ');
      ...
      begin
         id_sup_invoice := sqlDados.FieldByName('ID_SUP_INVOICE').AsInteger;
         id_supplier    := sqlDados.FieldByName('ID_SUPPLIER').AsInteger;
         id_company     := sqlDados.FieldByName('ID_COMPANY').AsInteger;
         date_invoice   := sqlDados.FieldByName('DATE_INVOICE').AsDateTime;
         invoice_id     := sqlDados.FieldByName('INVOICE_ID').AsString;
         salesrep       := sqlDados.FieldByName('SALESREP').AsString;
         freight        := sqlDados.FieldByName('FREIGHT').AsFloat;
         tax            := sqlDados.FieldByName('TAX').AsFloat;
         merchandise    := sqlDados.FieldByName('MERCHANDISE').AsFloat;
...
begin
   ...
   AreaSquareFeet     := pItemNF.AreaSquareFeet;
end;
...
begin
  ...
  begin
    ...
    Try
      ...
      sqlDados.SQL.Add(',AreaSquareFeet ');
      sqlDados.SQL.Add(',id_user)');
      sqlDados.SQL.Add(' Values ( ');
      ...
      sqlDados.SQL.Add(  QuotedStr( FloatToStr( AreaSquareFeet )) +  ',' );
...
procedure TInvoiceItem.setFAreaSquareFeet(const Value: Double);
begin
  FAreaSquareFeet := Value;
end;
...
begin
   ...
   begin
       ...
       Try
        ...
        sqldados.sql.add('Update TBSUP_INVOICEITEM ');
        ...
        sqlDados.SQL.Add(',AreaSquareFeet  = :AreaSquareFeet');
        sqlDados.SQL.Add(',rollnumber = :rollnumber ');
        sqlDados.SQL.Add(',dyelot     = :dyelot');
        sqlDados.SQL.Add(',unitprice  = :unitprice');
        sqlDados.SQL.Add(',amount     = :amount');
        sqlDados.SQL.Add(',id_user    = :id_user');
        sqlDados.SQL.Add(',upd_date   = :upd_date  ');
        sqlDados.SQL.Add(' Where  id_sup_invoiceitem = :id_sup_invoiceitem ');
        sqlDados.SQL.Add(' and id_sup_invoice = :id_sup_invoice');
        ...
        sqlDados.Params.ParamByName('qty').AsInteger            := qty;
        sqlDados.Params.ParamByName('totalarea').AsFloat        := totalarea;
        sqlDados.Params.ParamByName('AreaSquareFeet').AsFloat   := AreaSquareFeet;
        sqlDados.Params.ParamByName('rollnumber').AsString      := rollnumber;
        sqlDados.Params.ParamByName('dyelot').AsString          := dyelot;
        sqlDados.Params.ParamByName('unitprice').AsFloat        := unitprice;
        sqlDados.Params.ParamByName('amount').AsFloat           := amount;
        sqlDados.Params.ParamByName('id_user').AsInteger        := id_user;
        sqlDados.Params.ParamByName('upd_date').AsString        := FormatDateTime('mm/dd/yyyy hh:mm:ss', upd_date);
