object frmWorkOrder: TfrmWorkOrder
  Left = 0
  Top = 0
  Caption = 'Work Order'
  ClientHeight = 541
  ClientWidth = 1023
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object Panel27: TPanel
    Left = 0
    Top = 486
    Width = 1023
    Height = 36
    Align = alBottom
    BevelOuter = bvNone
    TabOrder = 0
    object ButSair: TcxButton
      Left = 39
      Top = 2
      Width = 36
      Height = 33
      Hint = 'Voltar (Alt+V)'
      Caption = '   &V'
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Blue'
      OptionsImage.Glyph.Data = {
        
        B4A1F0DFD7FF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      OptionsImage.Margin = 5
      TabOrder = 0
      OnClick = ButSairClick
    end
    object ButImprimir: TcxButton
      Left = 2
      Top = 2
      Width = 36
      Height = 33
      Hint = 'Visualizar Impress'#227'o (Alt+I)'
      Caption = '   &I'
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Blue'
      OptionsImage.Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      OptionsImage.Margin = 5
      TabOrder = 1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 522
    Width = 1023
    Height = 19
    Panels = <
      item
        Text = 'User'
        Width = 300
      end
      item
        Text = 'Date created'
        Width = 300
      end
      item
        Text = 'Date modified'
        Width = 300
      end>
  end
  object cxGrid2: TcxGrid
    Left = 0
    Top = 40
    Width = 1023
    Height = 446
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 2
    object cxGrid2DBTableView1: TcxGridDBTableView
      Navigator.Buttons.CustomButtons = <>
      Navigator.Visible = True
      DataController.DataSource = dsGrid
      DataController.KeyFieldNames = 'ID_SERVICE'
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <
        item
          Kind = skSum
          FieldName = 'DISCOUNT'
        end
        item
          Kind = skSum
          FieldName = 'TAX'
        end
        item
          Kind = skSum
          FieldName = 'SHIPPING'
        end
        item
          Kind = skSum
          FieldName = 'TOTAL'
        end
        item
          Kind = skSum
          FieldName = 'SUBTOTAL'
        end>
      DataController.Summary.SummaryGroups = <>
      FilterRow.Visible = True
      OptionsData.Deleting = False
      OptionsData.DeletingConfirmation = False
      OptionsData.Inserting = False
      OptionsView.Footer = True
      OptionsView.Indicator = True
      object cxGrid2DBTableView1ID_SERVICE: TcxGridDBColumn
        Caption = 'Work Order ID'
        DataBinding.FieldName = 'ID_SERVICE'
        Width = 80
      end
      object cxGrid2DBTableView1DT_SERVICE: TcxGridDBColumn
        Caption = 'Installation Date'
        DataBinding.FieldName = 'DT_SERVICE'
        Width = 89
      end
      object cxGrid2DBTableView1NAMEBUSINESS: TcxGridDBColumn
        Caption = 'Supplier'
        DataBinding.FieldName = 'NAMEBUSINESS'
        Width = 120
      end
      object cxGrid2DBTableView1NAME: TcxGridDBColumn
        Caption = 'Worker Name'
        DataBinding.FieldName = 'NAME'
        Width = 100
      end
      object cxGrid2DBTableView1TYPEJOB: TcxGridDBColumn
        Caption = 'Job Type'
        DataBinding.FieldName = 'TYPEJOB'
        Width = 76
      end
      object cxGrid2DBTableView1TOTALHOUR: TcxGridDBColumn
        Caption = 'Hour(s)'
        DataBinding.FieldName = 'TOTALHOUR'
        Width = 45
      end
      object cxGrid2DBTableView1STATUS: TcxGridDBColumn
        Caption = 'Status'
        DataBinding.FieldName = 'STATUS'
        PropertiesClassName = 'TcxComboBoxProperties'
        Properties.Items.Strings = (
          'PENDING'
          'COMPLETED'
          'CANCELED')
        Width = 100
      end
      object cxGrid2DBTableView1DT_FINISH: TcxGridDBColumn
        Caption = 'Finish Date'
        DataBinding.FieldName = 'DT_FINISH'
        Width = 100
      end
      object cxGrid2DBTableView1LAST_NAME: TcxGridDBColumn
        Caption = 'Customer LastName'
        DataBinding.FieldName = 'LAST_NAME'
        Width = 111
      end
      object cxGrid2DBTableView1FIRST_NAME: TcxGridDBColumn
        Caption = 'Customer First Name'
        DataBinding.FieldName = 'FIRST_NAME'
        Width = 111
      end
      object cxGrid2DBTableView1ADDRESS1: TcxGridDBColumn
        Caption = 'Address'
        DataBinding.FieldName = 'ADDRESS1'
        Width = 100
      end
      object cxGrid2DBTableView1COUNTY: TcxGridDBColumn
        Caption = 'County'
        DataBinding.FieldName = 'COUNTY'
        Width = 100
      end
      object cxGrid2DBTableView1ID_PROCESS: TcxGridDBColumn
        Caption = 'Invoice ID'
        DataBinding.FieldName = 'ID_PROCESS'
        Width = 100
      end
    end
    object cxGrid2Level1: TcxGridLevel
      GridView = cxGrid2DBTableView1
    end
  end
  object rgStatus: TcxRadioGroup
    Left = 0
    Top = 0
    Align = alTop
    Caption = 'Status'
    Properties.Columns = 4
    Properties.Items = <
      item
        Caption = 'Pending'
        Value = '1'
      end
      item
        Caption = 'Completed'
        Value = '2'
      end
      item
        Caption = 'Canceled'
        Value = '3'
      end>
    ItemIndex = 0
    TabOrder = 3
    OnClick = rgStatusClick
    Height = 40
    Width = 1023
  end
  object sqlGrid: TFDQuery
    BeforePost = sqlGridBeforePost
    Connection = DBDados.FDConnection
    SQL.Strings = (
      
        'SELECT A.ID_SERVICE, A.DT_SERVICE, G.NAMEBUSINESS, F.NAME, E.TYP' +
        'EJOB, E.TOTALHOUR, A.STATUS, A.DT_FINISH,  C.LAST_NAME, C.FIRST_' +
        'NAME, D.ADDRESS1, D.COUNTY,   A.ID_PROCESS'
      'FROM TBSERVICE A'
      
        'INNER JOIN TBPROCESS B ON B.ID_PROCESS = A.ID_PROCESS AND B.TABL' +
        'ENAME = A.TABLENAME AND A.TABLENAME = '#39'TBINVOICE'#39
      'INNER JOIN TBCUSTOMER C ON C.ID_CUSTOMER = B.ID_CUSTOMER'
      
        'LEFT OUTER JOIN TBADDRESS D ON D.ID_CUSTOMER = B.ID_CUSTOMER AND' +
        ' D.ID_ADDRESS = B.ID_ADDRESS'
      
        'INNER JOIN TBSERVICE_CREW E ON E.TABLENAME = A.TABLENAME AND E.I' +
        'D_PROCESS = A.ID_PROCESS'
      'INNER JOIN TBCONTRACTORS F ON F.ID_CONTRACTORS = E.ID_CONTRACTOR'
      'INNER JOIN TBSUPPLIER G ON G.ID_SUPPLIER = A.ID_SUPPLIER'
      'WHERE (1 = 1 )'
      '&WHERE1'
      '&WHERE2'
      '&WHERE3')
    Left = 416
    Top = 168
    MacroData = <
      item
        Value = Null
        Name = 'WHERE1'
        DataType = mdIdentifier
      end
      item
        Value = Null
        Name = 'WHERE2'
        DataType = mdIdentifier
      end
      item
        Value = Null
        Name = 'WHERE3'
        DataType = mdIdentifier
      end>
    object sqlGridID_SERVICE: TFDAutoIncField
      FieldName = 'ID_SERVICE'
      Origin = 'ID_SERVICE'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object sqlGridDT_SERVICE: TSQLTimeStampField
      FieldName = 'DT_SERVICE'
      Origin = 'DT_SERVICE'
    end
    object sqlGridNAMEBUSINESS: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'NAMEBUSINESS'
      Origin = 'NAMEBUSINESS'
      ProviderFlags = []
      ReadOnly = True
      Size = 100
    end
    object sqlGridNAME: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'NAME'
      Origin = 'NAME'
      ProviderFlags = []
      ReadOnly = True
      Size = 50
    end
    object sqlGridTYPEJOB: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'TYPEJOB'
      Origin = 'TYPEJOB'
      ProviderFlags = []
      ReadOnly = True
      FixedChar = True
      Size = 4
    end
    object sqlGridTOTALHOUR: TFloatField
      AutoGenerateValue = arDefault
      FieldName = 'TOTALHOUR'
      Origin = 'TOTALHOUR'
      ProviderFlags = []
      ReadOnly = True
    end
    object sqlGridSTATUS: TStringField
      FieldName = 'STATUS'
      Origin = 'STATUS'
      Size = 10
    end
    object sqlGridDT_FINISH: TSQLTimeStampField
      FieldName = 'DT_FINISH'
      Origin = 'DT_FINISH'
    end
    object sqlGridLAST_NAME: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'LAST_NAME'
      Origin = 'LAST_NAME'
      ProviderFlags = []
      ReadOnly = True
      Size = 50
    end
    object sqlGridFIRST_NAME: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'FIRST_NAME'
      Origin = 'FIRST_NAME'
      ProviderFlags = []
      ReadOnly = True
      Size = 30
    end
    object sqlGridADDRESS1: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'ADDRESS1'
      Origin = 'ADDRESS1'
      ProviderFlags = []
      ReadOnly = True
      Size = 400
    end
    object sqlGridCOUNTY: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'COUNTY'
      Origin = 'COUNTY'
      ProviderFlags = []
      ReadOnly = True
      Size = 80
    end
    object sqlGridID_PROCESS: TIntegerField
      FieldName = 'ID_PROCESS'
      Origin = 'ID_PROCESS'
    end
  end
  object dsGrid: TDataSource
    DataSet = sqlGrid
    Left = 416
    Top = 240
  end
end
