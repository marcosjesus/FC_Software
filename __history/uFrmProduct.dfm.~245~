object frmProduct: TfrmProduct
  Left = 0
  Top = 0
  Caption = 'Product'
  ClientHeight = 728
  ClientWidth = 1370
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsMDIChild
  KeyPreview = True
  OldCreateOrder = False
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label18: TLabel
    Left = 42
    Top = 22
    Width = 75
    Height = 14
    Caption = 'U$ Cost Value'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object cxPageControl: TcxPageControl
    Left = 0
    Top = 0
    Width = 1370
    Height = 673
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    Properties.ActivePage = cxTabSheetForm
    Properties.CustomButtons.Buttons = <>
    OnChange = cxPageControlChange
    ClientRectBottom = 669
    ClientRectLeft = 4
    ClientRectRight = 1366
    ClientRectTop = 24
    object cxTabSheetList: TcxTabSheet
      Caption = 'Products List'
      ImageIndex = 0
      object cxGrid1: TcxGrid
        Left = 0
        Top = 0
        Width = 1362
        Height = 645
        Align = alClient
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object cxGrid1DBTableView1: TcxGridDBTableView
          OnDblClick = cxGrid1DBTableView1DblClick
          Navigator.Buttons.CustomButtons = <>
          DataController.DataSource = dsGrid
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          FilterRow.Visible = True
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsView.GroupByBox = False
          OptionsView.Indicator = True
          object cxGrid1DBTableView1ID_PRODUCT: TcxGridDBColumn
            Caption = 'Product ID'
            DataBinding.FieldName = 'ID_PRODUCT'
          end
          object cxGrid1DBTableView1MATERIAL: TcxGridDBColumn
            Caption = 'Product Type'
            DataBinding.FieldName = 'MATERIAL'
            Width = 100
          end
          object cxGrid1DBTableView1STYLE: TcxGridDBColumn
            Caption = 'Style'
            DataBinding.FieldName = 'STYLE'
            Width = 100
          end
          object cxGrid1DBTableView1manufactore: TcxGridDBColumn
            Caption = 'Manufacture'
            DataBinding.FieldName = 'MANUFACTURE'
            Width = 100
          end
          object cxGrid1DBTableView1PRODUCT_STYLE: TcxGridDBColumn
            Caption = 'Style'
            DataBinding.FieldName = 'PRODUCT_STYLE'
            Width = 100
          end
          object cxGrid1DBTableView1PRODUCT_STYLE_NAME: TcxGridDBColumn
            Caption = 'Style Name'
            DataBinding.FieldName = 'PRODUCT_STYLE_NAME'
            Width = 100
          end
          object cxGrid1DBTableView1COLOR: TcxGridDBColumn
            Caption = 'Color'
            DataBinding.FieldName = 'COLOR'
            Width = 100
          end
          object cxGrid1DBTableView1COLOR_NAME: TcxGridDBColumn
            Caption = 'Color Name'
            DataBinding.FieldName = 'COLOR_NAME'
            Width = 100
          end
          object cxGrid1DBTableView1LOC_SECTION: TcxGridDBColumn
            Caption = 'Section'
            DataBinding.FieldName = 'LOC_SECTION'
            Width = 100
          end
          object cxGrid1DBTableView1LOC_POSITION: TcxGridDBColumn
            Caption = 'Position'
            DataBinding.FieldName = 'LOC_POSITION'
            Width = 100
          end
          object cxGrid1DBTableView1AreaSquareFeetPerBox: TcxGridDBColumn
            Caption = 'Quanty Per Carton'
            DataBinding.FieldName = 'AreaSquareFeetPerBox'
            Width = 100
          end
          object cxGrid1DBTableView1QTY: TcxGridDBColumn
            Caption = 'Numbers of Carton'
            DataBinding.FieldName = 'QTY'
            Width = 100
          end
          object cxGrid1DBTableView1TotalAreaSquareFeet: TcxGridDBColumn
            Caption = 'Total Quantity'
            DataBinding.FieldName = 'TotalAreaSquareFeet'
            Width = 100
          end
        end
        object cxGrid1Level1: TcxGridLevel
          GridView = cxGrid1DBTableView1
        end
      end
    end
    object cxTabSheetForm: TcxTabSheet
      Caption = 'Registration Form'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageIndex = 1
      ParentFont = False
      object Panel1: TPanel
        Tag = -2
        Left = 0
        Top = 0
        Width = 1362
        Height = 27
        Align = alTop
        Alignment = taLeftJustify
        BevelInner = bvLowered
        Caption = '     ID'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        object Label1: TLabel
          Left = 317
          Top = 7
          Width = 367
          Height = 14
          Caption = 
            'Please fill out this form as completely and accurantely as possi' +
            'ble '
        end
        object edtCodigo: TcxCurrencyEdit
          Left = 38
          Top = 3
          Enabled = False
          Properties.AssignedValues.DisplayFormat = True
          Properties.DecimalPlaces = 0
          Style.Color = clInactiveCaption
          TabOrder = 0
          Width = 51
        end
      end
      object PgCPrincipal: TcxPageControl
        Tag = -1
        Left = 0
        Top = 27
        Width = 1362
        Height = 618
        Align = alClient
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        Properties.ActivePage = TbSFicha
        Properties.CustomButtons.Buttons = <>
        LookAndFeel.Kind = lfOffice11
        LookAndFeel.NativeStyle = False
        LookAndFeel.SkinName = 'Office2007Blue'
        ClientRectBottom = 612
        ClientRectLeft = 3
        ClientRectRight = 1356
        ClientRectTop = 26
        object TbSFicha: TcxTabSheet
          Caption = 'Registration Form'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ImageIndex = 0
          ParentFont = False
          object Image1: TImage
            Left = 988
            Top = 22
            Width = 349
            Height = 287
            Stretch = True
          end
          object Panel2: TPanel
            Tag = -1
            Left = 9
            Top = 6
            Width = 712
            Height = 38
            Alignment = taLeftJustify
            BevelInner = bvLowered
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 0
            object Label10: TLabel
              Left = 43
              Top = 9
              Width = 69
              Height = 14
              Caption = 'Manufacture'
            end
            object edtSupplier: TEditBusca
              Left = 124
              Top = 6
              Width = 563
              Height = 22
              CharCase = ecUpperCase
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              TabOrder = 0
              OnClick = edtSupplierClick
              ClickOnArrow = True
              ClickOnReturn = False
              bs_Conexao = DBDados.FDConnection
              bs_HeightForm = 0
              bs_WidthForm = 0
              bs_SetCPF = False
              bs_SetCNPJ = False
              bs_SetPlaca = False
              bs_LoadConsulta = False
              bs_Distinct = False
              bs_SetColor = False
              bs_NomeCor = clBlack
              bs_IndiceCampo = 0
              bs_Imagem = False
              bs_HideTop = False
              bs_Top100 = False
              bs_Script = False
            end
          end
          object Panel3: TPanel
            Tag = -1
            Left = 9
            Top = 79
            Width = 712
            Height = 32
            Alignment = taLeftJustify
            BevelInner = bvLowered
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 2
            object Label6: TLabel
              Left = 85
              Top = 11
              Width = 27
              Height = 14
              Caption = 'Style'
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label12: TLabel
              Left = 364
              Top = 9
              Width = 61
              Height = 14
              Caption = 'Style Name'
            end
            object edProductStyle: TEdit
              Left = 124
              Top = 6
              Width = 197
              Height = 22
              CharCase = ecUpperCase
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              MaxLength = 30
              ParentFont = False
              TabOrder = 0
            end
            object edtProductStyleName: TEdit
              Left = 431
              Top = 6
              Width = 256
              Height = 22
              CharCase = ecUpperCase
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              MaxLength = 50
              ParentFont = False
              TabOrder = 1
            end
          end
          object Panel5: TPanel
            Tag = -1
            Left = 9
            Top = 144
            Width = 713
            Height = 67
            Alignment = taLeftJustify
            BevelInner = bvLowered
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 4
            object Label2: TLabel
              Left = 16
              Top = 18
              Width = 157
              Height = 14
              Caption = 'Internal Product Description/'
            end
            object Label16: TLabel
              Left = 32
              Top = 38
              Width = 107
              Height = 14
              Caption = 'Service Description'
            end
            object memProduct: TMemo
              Left = 182
              Top = 7
              Width = 506
              Height = 57
              CharCase = ecUpperCase
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              MaxLength = 500
              ParentFont = False
              TabOrder = 0
            end
          end
          object rgTaxable: TRadioGroup
            Left = 740
            Top = 3
            Width = 229
            Height = 41
            Caption = 'Taxble'
            Columns = 2
            ItemIndex = 0
            Items.Strings = (
              'Yes'
              'No')
            TabOrder = 5
          end
          object Panel9: TPanel
            Tag = -1
            Left = 740
            Top = 93
            Width = 229
            Height = 104
            Alignment = taLeftJustify
            BevelInner = bvLowered
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 3
            object Label13: TLabel
              Left = 13
              Top = 7
              Width = 82
              Height = 14
              Caption = 'ID Manufacture'
            end
            object Label11: TLabel
              Left = 13
              Top = 53
              Width = 21
              Height = 14
              Caption = 'SKU'
            end
            object edtSupplierItem: TEdit
              Left = 13
              Top = 25
              Width = 198
              Height = 22
              CharCase = ecUpperCase
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              MaxLength = 30
              ParentFont = False
              TabOrder = 0
            end
            object edtSKU: TEdit
              Left = 13
              Top = 71
              Width = 198
              Height = 22
              CharCase = ecUpperCase
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              MaxLength = 20
              ParentFont = False
              TabOrder = 1
            end
          end
          object rgActive: TRadioGroup
            Left = 740
            Top = 50
            Width = 229
            Height = 41
            Caption = 'Actived'
            Columns = 2
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ItemIndex = 0
            Items.Strings = (
              'Yes'
              'No')
            ParentFont = False
            TabOrder = 6
          end
          object Panel7: TPanel
            Left = 9
            Top = 215
            Width = 712
            Height = 221
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 7
            object Panel4: TPanel
              Left = 1
              Top = 1
              Width = 710
              Height = 219
              Align = alClient
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
              TabOrder = 0
              object Label14: TLabel
                Left = 385
                Top = 77
                Width = 140
                Height = 13
                Caption = 'Quantity Per Carton/Roll'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWindowText
                Font.Height = -11
                Font.Name = 'MS Sans Serif'
                Font.Style = [fsBold]
                ParentFont = False
              end
              object Label3: TLabel
                Left = 526
                Top = 98
                Width = 22
                Height = 13
                Caption = 'sqft'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWindowText
                Font.Height = -11
                Font.Name = 'MS Sans Serif'
                Font.Style = [fsBold]
                ParentFont = False
              end
              object Label8: TLabel
                Left = 567
                Top = 126
                Width = 103
                Height = 13
                Caption = 'Total Square Feet'
              end
              object Label21: TLabel
                Left = 19
                Top = 158
                Width = 70
                Height = 13
                Caption = 'Roll Number'
              end
              object Label22: TLabel
                Left = 220
                Top = 158
                Width = 45
                Height = 13
                Caption = 'Dye Lot'
              end
              object Label23: TLabel
                Left = 18
                Top = 110
                Width = 34
                Height = 13
                Caption = 'Width'
              end
              object Label26: TLabel
                Left = 220
                Top = 110
                Width = 40
                Height = 13
                Caption = 'Length'
              end
              object Label19: TLabel
                Left = 567
                Top = 38
                Width = 111
                Height = 13
                Caption = 'U$ Cost Value Feet'
              end
              object Shape1: TShape
                Left = 3
                Top = 94
                Width = 358
                Height = 2
              end
              object Label4: TLabel
                Left = 220
                Top = 21
                Width = 81
                Height = 13
                Caption = 'Number CTNS'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWindowText
                Font.Height = -11
                Font.Name = 'MS Sans Serif'
                Font.Style = [fsBold]
                ParentFont = False
              end
              object Label7: TLabel
                Left = 18
                Top = 21
                Width = 82
                Height = 13
                Caption = 'U$ Cost Value'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clWindowText
                Font.Height = -11
                Font.Name = 'MS Sans Serif'
                Font.Style = [fsBold]
                ParentFont = False
              end
              object Shape2: TShape
                Left = 554
                Top = 0
                Width = 2
                Height = 219
              end
              object Shape3: TShape
                Left = 360
                Top = 0
                Width = 2
                Height = 219
              end
              object edtMskQty: TcxMaskEdit
                Left = 219
                Top = 39
                TabOrder = 1
                OnExit = edtCostValueExit
                OnKeyPress = edtMskQtyKeyPress
                Width = 130
              end
              object edtAreaSquareFeetPerBox: TEdit
                Left = 385
                Top = 95
                Width = 130
                Height = 22
                Font.Charset = ANSI_CHARSET
                Font.Color = clWindowText
                Font.Height = -11
                Font.Name = 'Arial'
                Font.Style = [fsBold]
                ParentFont = False
                TabOrder = 2
                OnExit = edtCostValueExit
                OnKeyPress = edtAreaSquareFeetPerBoxKeyPress
              end
              object edtCostValue: TcxMaskEdit
                Left = 18
                Top = 39
                TabOrder = 0
                OnExit = edtCostValueExit
                OnKeyPress = edtCostValueKeyPress
                Width = 130
              end
              object edtCostValuePerFeet: TcxCurrencyEdit
                Left = 567
                Top = 57
                Style.Color = clWhite
                TabOrder = 3
                Width = 130
              end
              object edtTotalAreaSquareFeet: TEdit
                Left = 567
                Top = 144
                Width = 130
                Height = 22
                Color = clWhite
                Font.Charset = ANSI_CHARSET
                Font.Color = clWindowText
                Font.Height = -11
                Font.Name = 'Arial'
                Font.Style = [fsBold]
                ParentFont = False
                TabOrder = 4
              end
              object edtrollnumber: TEdit
                Left = 18
                Top = 175
                Width = 130
                Height = 21
                MaxLength = 20
                TabOrder = 7
              end
              object edtdyelot: TEdit
                Left = 219
                Top = 175
                Width = 130
                Height = 21
                MaxLength = 20
                TabOrder = 8
              end
              object edtWidth: TcxCurrencyEdit
                Left = 18
                Top = 128
                TabOrder = 5
                OnExit = edtWidthExit
                Width = 130
              end
              object edtHeight: TcxCurrencyEdit
                Left = 219
                Top = 128
                TabOrder = 6
                OnExit = edtHeightExit
                Width = 130
              end
            end
          end
          object Panel8: TPanel
            Tag = -1
            Left = 9
            Top = 45
            Width = 712
            Height = 32
            Alignment = taLeftJustify
            BevelInner = bvLowered
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 1
            object Label5: TLabel
              Left = 353
              Top = 12
              Width = 72
              Height = 14
              Caption = 'Product Type'
            end
            object Label9: TLabel
              Left = 39
              Top = 12
              Width = 73
              Height = 14
              Caption = 'Product Style'
            end
            object cxLookupComboBoxType: TcxLookupComboBox
              Left = 431
              Top = 6
              Properties.KeyFieldNames = 'ID_TYPEBRAND'
              Properties.ListColumns = <
                item
                  FieldName = 'DESCRIPTION'
                end>
              Properties.ListSource = dsType
              Properties.OnChange = cxLookupComboBoxTypePropertiesChange
              TabOrder = 0
              OnClick = cxLookupComboBoxTypePropertiesChange
              Width = 256
            end
            object cxLookupComboBoxStyle: TcxLookupComboBox
              Left = 124
              Top = 6
              Properties.KeyFieldNames = 'ID_TYPEBRAND'
              Properties.ListColumns = <
                item
                  FieldName = 'DESCRIPTION'
                end>
              Properties.ListSource = DSPRODUCTSTYLE
              Properties.OnChange = cxLookupComboBoxTypePropertiesChange
              TabOrder = 1
              OnClick = cxLookupComboBoxTypePropertiesChange
              Width = 197
            end
          end
          object Panel10: TPanel
            Tag = -1
            Left = 9
            Top = 112
            Width = 712
            Height = 32
            Alignment = taLeftJustify
            BevelInner = bvLowered
            Font.Charset = ANSI_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 8
            object Label15: TLabel
              Left = 82
              Top = 9
              Width = 30
              Height = 14
              Caption = 'Color'
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label20: TLabel
              Left = 361
              Top = 9
              Width = 64
              Height = 14
              Caption = 'Color Name'
            end
            object edtColor: TEdit
              Left = 124
              Top = 6
              Width = 197
              Height = 22
              CharCase = ecUpperCase
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              MaxLength = 30
              ParentFont = False
              TabOrder = 0
            end
            object edtColorName: TEdit
              Left = 431
              Top = 6
              Width = 256
              Height = 22
              CharCase = ecUpperCase
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              MaxLength = 50
              ParentFont = False
              TabOrder = 1
            end
          end
          object GroupBox1: TGroupBox
            Left = 740
            Top = 202
            Width = 229
            Height = 128
            Caption = 'Location'
            TabOrder = 9
            object Label24: TLabel
              Left = 14
              Top = 23
              Width = 41
              Height = 14
              Caption = 'Section'
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label25: TLabel
              Left = 17
              Top = 71
              Width = 46
              Height = 13
              Caption = 'Position'
            end
            object edtSection: TEdit
              Left = 14
              Top = 43
              Width = 197
              Height = 22
              CharCase = ecUpperCase
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              MaxLength = 10
              ParentFont = False
              TabOrder = 0
            end
            object edtPosition: TEdit
              Left = 13
              Top = 90
              Width = 198
              Height = 22
              CharCase = ecUpperCase
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              MaxLength = 10
              ParentFont = False
              TabOrder = 1
            end
          end
          object btnSaveImage: TcxButton
            Left = 988
            Top = 319
            Width = 117
            Height = 37
            Caption = 'Save Image'
            TabOrder = 10
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            OnClick = btnSaveImageClick
          end
        end
      end
    end
  end
  object Panel27: TPanel
    Left = 0
    Top = 673
    Width = 1370
    Height = 36
    Align = alBottom
    BevelOuter = bvNone
    TabOrder = 1
    object ButNovo: TcxButton
      Left = 0
      Top = 2
      Width = 36
      Height = 33
      Hint = 'Novo (Alt+N)'
      Caption = '  &N'
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Blue'
      OptionsImage.Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      OptionsImage.Margin = 5
      TabOrder = 0
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
      OnClick = ButNovoClick
    end
    object ButAlterar: TcxButton
      Left = 35
      Top = 2
      Width = 36
      Height = 33
      Hint = 'Alterar (Alt+A)'
      Caption = '      &A'
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Blue'
      OptionsImage.Glyph.Data = {
        
        2323232323232323232323232323232323232323232323232323}
      OptionsImage.Margin = 7
      TabOrder = 1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -8
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      WordWrap = True
      OnClick = ButAlterarClick
    end
    object ButExcluir: TcxButton
      Left = 70
      Top = 2
      Width = 36
      Height = 33
      Hint = 'Excluir (Alt+E)'
      Caption = '   &E'
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Blue'
      OptionsImage.Glyph.Data = {
        
        00FFFF00FFFF00FFFAC088FAC088FAC088FAC088FF00FFFF00FF}
      OptionsImage.Margin = 5
      TabOrder = 2
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -8
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      OnClick = ButExcluirClick
    end
    object ButSair: TcxButton
      Left = 140
      Top = 2
      Width = 36
      Height = 33
      Hint = 'Voltar (Alt+V)'
      Caption = '   &V'
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Blue'
      OptionsImage.Glyph.Data = {
        
        B4A1F0DFD7FF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      OptionsImage.Margin = 5
      TabOrder = 3
      OnClick = ButSairClick
    end
    object ButSalvar: TcxButton
      Left = 280
      Top = 2
      Width = 36
      Height = 33
      Hint = 'Salvar ( Alt + S )'
      Caption = '   &S'
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Blue'
      OptionsImage.Glyph.Data = {
        
        6666666666666666666666666666666666666666666666666666}
      OptionsImage.Margin = 5
      TabOrder = 4
      OnClick = ButSalvarClick
    end
    object ButCancelar: TcxButton
      Left = 322
      Top = 2
      Width = 36
      Height = 33
      Hint = 'Cancelar (Alt + C)'
      BiDiMode = bdLeftToRight
      Caption = '   &C'
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Blue'
      OptionsImage.Glyph.Data = {
        
        0101010101010101010101010101010101010101010101010101}
      OptionsImage.Margin = 5
      ParentBiDiMode = False
      TabOrder = 5
      OnClick = ButCancelarClick
    end
    object ButImprimir: TcxButton
      Left = 105
      Top = 2
      Width = 36
      Height = 33
      Hint = 'Exportar para Excel'
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Blue'
      OptionsImage.Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 6
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = ButImprimirClick
    end
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 709
    Width = 1370
    Height = 19
    Panels = <
      item
        Text = 'User'
        Width = 300
      end
      item
        Text = 'Date created'
        Width = 300
      end
      item
        Text = 'Date modified'
        Width = 300
      end>
  end
  object sqlGrid: TFDQuery
    Connection = DBDados.FDConnection
    SQL.Strings = (
      ''
      'SELECT TP.DESCRIPTION AS MATERIAL, '
      '       TS.DESCRIPTION AS STYLE,'
      #9'   S.NAMEBUSINESS AS MANUFACTURE,'
      #9'   P.PRODUCT_STYLE,'
      #9'   P.PRODUCT_STYLE_NAME,'
      #9'   P.COLOR,'
      #9'   P.COLOR_NAME,'
      #9'   P.LOC_SECTION,'
      #9'   P.LOC_POSITION,'
      #9'   P.AreaSquareFeetPerBox,'
      #9'   P.QTY,'
      #9'   P.TOTALAREASQUAREFEET,'
      #9'   P.COST_VALUE,'
      #9'   P.CostValuePerFeet,'
      #9'   P.SKU,'
      #9'   P.ID_TYPE,'
      #9'   P.TYPEOFPRODUCT,'
      #9'   P.ACTIVE,'
      #9'   P.ADD_DATE,'
      #9'   P.UPD_DATE,'
      #9'   P.AreaSquareYardPerBox,'
      #9'   P.TotalAreaSquareYard,'
      #9'   P.rollnumber,'
      #9'   P.rollnumberdialog,'
      #9'   P.ID_USER,'
      #9'   P.ID_PRODUCT,'
      #9'   P.ID_SUPPLIER,'
      #9'   P.SUPPLIER_ITEM,'
      #9'   P.TAXBLE'
      #9'   FROM TBPRODUCT P'
      'INNER JOIN TBSUPPLIER S ON S.ID_SUPPLIER = P.ID_SUPPLIER'
      'LEFT OUTER JOIN TBTYPEBRAND TP ON TP.ID_TYPEBRAND = P.ID_TYPE'
      'LEFT OUTER JOIN TBTYPEBRAND TS ON TS.ID_TYPEBRAND = P.STYLE'
      'ORDER BY S.NAMEBUSINESS, P.PRODUCT_NAME')
    Left = 844
    Top = 552
    object sqlGridMATERIAL: TStringField
      FieldName = 'MATERIAL'
      Origin = 'MATERIAL'
      Size = 50
    end
    object sqlGridSTYLE: TStringField
      FieldName = 'STYLE'
      Origin = 'STYLE'
      Size = 50
    end
    object sqlGridMANUFACTURE: TStringField
      FieldName = 'MANUFACTURE'
      Origin = 'MANUFACTURE'
      Size = 100
    end
    object sqlGridPRODUCT_STYLE: TStringField
      FieldName = 'PRODUCT_STYLE'
      Origin = 'PRODUCT_STYLE'
      Size = 30
    end
    object sqlGridPRODUCT_STYLE_NAME: TStringField
      FieldName = 'PRODUCT_STYLE_NAME'
      Origin = 'PRODUCT_STYLE_NAME'
      Size = 50
    end
    object sqlGridCOLOR: TStringField
      FieldName = 'COLOR'
      Origin = 'COLOR'
      Size = 30
    end
    object sqlGridCOLOR_NAME: TStringField
      FieldName = 'COLOR_NAME'
      Origin = 'COLOR_NAME'
      Size = 50
    end
    object sqlGridLOC_SECTION: TStringField
      FieldName = 'LOC_SECTION'
      Origin = 'LOC_SECTION'
      Size = 10
    end
    object sqlGridLOC_POSITION: TStringField
      FieldName = 'LOC_POSITION'
      Origin = 'LOC_POSITION'
      Size = 10
    end
    object sqlGridAreaSquareFeetPerBox: TFloatField
      FieldName = 'AreaSquareFeetPerBox'
      Origin = 'AreaSquareFeetPerBox'
    end
    object sqlGridQTY: TFloatField
      FieldName = 'QTY'
      Origin = 'QTY'
    end
    object sqlGridTOTALAREASQUAREFEET: TFloatField
      FieldName = 'TOTALAREASQUAREFEET'
      Origin = 'TOTALAREASQUAREFEET'
    end
    object sqlGridCOST_VALUE: TBCDField
      FieldName = 'COST_VALUE'
      Origin = 'COST_VALUE'
      Precision = 15
      Size = 2
    end
    object sqlGridCostValuePerFeet: TFloatField
      FieldName = 'CostValuePerFeet'
      Origin = 'CostValuePerFeet'
    end
    object sqlGridSKU: TStringField
      FieldName = 'SKU'
      Origin = 'SKU'
    end
    object sqlGridID_TYPE: TIntegerField
      FieldName = 'ID_TYPE'
      Origin = 'ID_TYPE'
    end
    object sqlGridTYPEOFPRODUCT: TStringField
      FieldName = 'TYPEOFPRODUCT'
      Origin = 'TYPEOFPRODUCT'
      Size = 7
    end
    object sqlGridACTIVE: TStringField
      FieldName = 'ACTIVE'
      Origin = 'ACTIVE'
      FixedChar = True
      Size = 1
    end
    object sqlGridADD_DATE: TSQLTimeStampField
      FieldName = 'ADD_DATE'
      Origin = 'ADD_DATE'
    end
    object sqlGridUPD_DATE: TSQLTimeStampField
      FieldName = 'UPD_DATE'
      Origin = 'UPD_DATE'
    end
    object sqlGridAreaSquareYardPerBox: TFloatField
      FieldName = 'AreaSquareYardPerBox'
      Origin = 'AreaSquareYardPerBox'
    end
    object sqlGridTotalAreaSquareYard: TFloatField
      FieldName = 'TotalAreaSquareYard'
      Origin = 'TotalAreaSquareYard'
    end
    object sqlGridrollnumber: TStringField
      FieldName = 'rollnumber'
      Origin = 'rollnumber'
      Size = 40
    end
    object sqlGridrollnumberdialog: TStringField
      FieldName = 'rollnumberdialog'
      Origin = 'rollnumberdialog'
      Size = 40
    end
    object sqlGridID_USER: TIntegerField
      FieldName = 'ID_USER'
      Origin = 'ID_USER'
    end
    object sqlGridID_PRODUCT: TIntegerField
      FieldName = 'ID_PRODUCT'
      Origin = 'ID_PRODUCT'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object sqlGridID_SUPPLIER: TIntegerField
      FieldName = 'ID_SUPPLIER'
      Origin = 'ID_SUPPLIER'
    end
    object sqlGridSUPPLIER_ITEM: TStringField
      FieldName = 'SUPPLIER_ITEM'
      Origin = 'SUPPLIER_ITEM'
      Size = 30
    end
    object sqlGridTAXBLE: TStringField
      FieldName = 'TAXBLE'
      Origin = 'TAXBLE'
      FixedChar = True
      Size = 1
    end
  end
  object dsGrid: TDataSource
    DataSet = sqlGrid
    Left = 908
    Top = 544
  end
  object SaveDialog: TSaveDialog
    DefaultExt = '*.xlsx'
    Filter = 'Arquivos Excel|*.xlsx'
    Left = 782
    Top = 552
  end
  object DSPRODUCTSTYLE: TDataSource
    DataSet = TBPRODUCTSTYLE
    Left = 463
    Top = 557
  end
  object dsType: TDataSource
    DataSet = TBTYPE
    Left = 727
    Top = 557
  end
  object TBPRODUCTSTYLE: TFDTable
    Filtered = True
    Filter = 'CATEGORY='#39'STYLE'#39
    IndexFieldNames = 'ID_TYPEBRAND'
    Connection = DBDados.FDConnection
    UpdateOptions.UpdateTableName = 'FLOORDB.dbo.TBTYPEBRAND'
    TableName = 'FLOORDB.dbo.TBTYPEBRAND'
    Left = 591
    Top = 557
    object TBPRODUCTSTYLEID_TYPEBRAND: TFDAutoIncField
      FieldName = 'ID_TYPEBRAND'
      Origin = 'ID_TYPEBRAND'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object TBPRODUCTSTYLECATEGORY: TStringField
      FieldName = 'CATEGORY'
      Origin = 'CATEGORY'
      Size = 15
    end
    object TBPRODUCTSTYLEDESCRIPTION: TStringField
      FieldName = 'DESCRIPTION'
      Origin = 'DESCRIPTION'
      Size = 50
    end
    object TBPRODUCTSTYLEADD_DATE: TDateField
      FieldName = 'ADD_DATE'
      Origin = 'ADD_DATE'
    end
    object TBPRODUCTSTYLEUPD_DATE: TDateField
      FieldName = 'UPD_DATE'
      Origin = 'UPD_DATE'
    end
    object TBPRODUCTSTYLEID_USER: TIntegerField
      FieldName = 'ID_USER'
      Origin = 'ID_USER'
    end
    object TBPRODUCTSTYLEPROFIT_MIN: TFloatField
      FieldName = 'PROFIT_MIN'
      Origin = 'PROFIT_MIN'
    end
    object TBPRODUCTSTYLEPROFIT_REGULAR: TFloatField
      FieldName = 'PROFIT_REGULAR'
      Origin = 'PROFIT_REGULAR'
    end
  end
  object TBTYPE: TFDTable
    Filtered = True
    Filter = 'CATEGORY='#39'PRODUCT TYPE'#39
    IndexFieldNames = 'ID_TYPEBRAND'
    Connection = DBDados.FDConnection
    UpdateOptions.UpdateTableName = 'FLOORDB.dbo.TBTYPEBRAND'
    TableName = 'FLOORDB.dbo.TBTYPEBRAND'
    Left = 663
    Top = 557
    object TBTYPEID_TYPEBRAND: TFDAutoIncField
      FieldName = 'ID_TYPEBRAND'
      Origin = 'ID_TYPEBRAND'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object TBTYPECATEGORY: TStringField
      FieldName = 'CATEGORY'
      Origin = 'CATEGORY'
      Size = 15
    end
    object TBTYPEDESCRIPTION: TStringField
      FieldName = 'DESCRIPTION'
      Origin = 'DESCRIPTION'
      Size = 50
    end
    object TBTYPEADD_DATE: TDateField
      FieldName = 'ADD_DATE'
      Origin = 'ADD_DATE'
    end
    object TBTYPEUPD_DATE: TDateField
      FieldName = 'UPD_DATE'
      Origin = 'UPD_DATE'
    end
    object TBTYPEID_USER: TIntegerField
      FieldName = 'ID_USER'
      Origin = 'ID_USER'
    end
    object TBTYPEPROFIT_MIN: TFloatField
      FieldName = 'PROFIT_MIN'
      Origin = 'PROFIT_MIN'
    end
    object TBTYPEPROFIT_REGULAR: TFloatField
      FieldName = 'PROFIT_REGULAR'
      Origin = 'PROFIT_REGULAR'
    end
  end
  object OpenPictureDialog: TOpenPictureDialog
    DefaultExt = '*.jpg'
    Left = 1119
    Top = 229
  end
  object SavePictureDialog: TSavePictureDialog
    Left = 1104
    Top = 296
  end
  object sqlImage: TFDQuery
    Connection = DBDados.FDConnection
    SQL.Strings = (
      'select * from tbimage where id_product = :id_product')
    Left = 1119
    Top = 165
    ParamData = <
      item
        Name = 'ID_PRODUCT'
        DataType = ftInteger
        ADDataType = dtInt32
        ParamType = ptInput
      end>
    object sqlImageID_IMAGE: TFDAutoIncField
      FieldName = 'ID_IMAGE'
      Origin = 'ID_IMAGE'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object sqlImageiD_PRODUCT: TIntegerField
      FieldName = 'iD_PRODUCT'
      Origin = 'iD_PRODUCT'
    end
    object sqlImageImagem: TBlobField
      FieldName = 'Imagem'
      Origin = 'Imagem'
      Size = 2147483647
    end
  end
end
