...
begin
  ...
  sqlBank.Params.ParamByName('ID_USER').AsInteger := DBDados.varID_USER;
  sqlBank.Open;
  sqlBank.First;
...
begin
  ...
  sqlUsuario.Params.ParamByName('ID_USER').AsInteger     :=  DBDados.varID_USER;
  ...
  begin
      if edtApelido.text = '' then begin
        Mens_MensInf( DBDados.RetornaMensagem('MEN_SELECTFILE') + ' - APELIDO' );
        edtApelido.SetFocus;
        Exit;
      end;
      ...
      if edtPathSalvar.text = '' then begin
        Mens_MensInf( DBDados.RetornaMensagem('MEN_SELECTFILE') );
      ...
      begin
         ...
         sqlAux.Close;
         sqlAux.SQL.Clear;
         sqlAux.SQL.Add('Insert Into TBTransacao (ID_USER, ID_BANKING, Tipo, DataTransacao, Favorecido,  Valor, CentroDeCusto,ID_USERBANK, MODO, FORECAST) Values ( ');
         sqlAux.SQL.Add(':ID_USER, :ID_BANKING, :Tipo, :DataTransacao, :Favorecido,  :Valor, :CentroDeCusto,:ID_USERBANK, :MODO, :FORECAST) ');
         sqlAux.Params.ParamByName('ID_USER').AsInteger      := DBDados.varID_USER;
  ...
  sqlUsuarioBanco.Params.ParamByName('ID_USER').AsInteger := DBDados.varID_USER;
  sqlUsuarioBanco.Open;

  DBDados.BuscaCaminhodoArquivo;
...
begin
  ...
  sqlUsuario.Close;
  sqlUsuario.SQL.Clear;
  sqlUsuario.SQL.Add('Select ID_USER, FULLNAME, EMAIL, PWD, PHONE_NUMBER From TbUser Where ID_User = :ID_User');
  sqlUsuario.Params.ParamByName('ID_User').AsInteger := DBDados.varID_USER;
  sqlUsuario.Open;

  edtID.Text       := sqlUsuario.FieldByName('ID_USER').AsString;
  ...
  sqlBank.Close;
  sqlBank.sql.Clear;
  sqlBank.SQL.Add('Select ID_BANKING, NAME from TBBANKING order by ID_BANKING');
  sqlBank.Open;
  sqlBank.First;
