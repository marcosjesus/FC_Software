...
begin
  ...
  try
     ...
     scrErro.lbltipo.Caption := Dados.RetornaMensagem('MEN_MENS_ERRO');
...
begin
  ...
  scrErro := TscrErro.Create ( Application );
  try
     scrErro.lbltipo.Caption := Dados.RetornaMensagem('MEN_MENS_INFO');
     scrErro.spbOk.Visible   := True ;
     scrErro.spbYes.Visible  := False ;
     scrErro.spbNo.Visible   := False ;
     scrErro.ButOpc1.Visible := False;
...
begin
  ...
  If ( Mensagem = '' ) Then
    ...
    scrErro := TscrErro.Create ( Application );
  try
     scrErro.lbltipo.Caption := Dados.RetornaMensagem('MEN_MENS_CONFI');
...
//     scrErro.Left := 226;
     ...
     scrErro.Caption         := Dados.RetornaMensagem('MEN_MENS_OPC');
