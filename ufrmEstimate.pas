...
uses
  ...
  Vcl.CheckLst, Vcl.Samples.Spin, Vcl.DBCtrls, Vcl.ExtDlgs;
...
begin
 ...
 if TBHeader = ESTIMATE_HEADER then
     varProcessName := 'Estimate #: ' + ZeroLeft(sqlProcessID_PROCESS.AsString,7)
...
begin
 ...
 if TBHeader = ESTIMATE_HEADER then
     varProcessName := 'Item Estimate #: ' + sqlProcessItemID_PRODUCT.AsString
 else if TBHeader = ORDER_HEADER then
     varProcessName := 'Item Order #: ' + sqlProcessItemID_PRODUCT.AsString;
...
begin
  ...
  if TBHeader = ESTIMATE_HEADER then
     LblProcess.Caption := 'Estimate Nº :'
  else if TBHeader = ORDER_HEADER Then
...
begin
  sqlProcessItem.Close;
  sqlProcessItem.Params.ParamByName('ID_PROCESS').AsInteger := varNewKey;
...
begin
  sqlTerms.Close;
  sqlTerms.Params.ParamByName('ID_PROCESS').AsInteger := sqlProcessItemID_PROCESS.AsInteger;
...
begin
   ...
   sqlProcessItem.Close;
   sqlProcessItem.Params.ParamByName('ID_PROCESS').AsInteger := sqlProcessID_PROCESS.AsInteger;
   sqlProcessItem.Params.ParamByName('TABLENAME').AsString   := TBItem;
   sqlProcessItem.Open;
     ...
     begin
        ...
        Try
          ...
          begin
              ...
              varSqlDados.Open;
              if not varSqlDados.IsEmpty  then
              begin
...
begin
  ...
  begin
    if  edtProduto.bs_KeyValues[24] = 'PRODUCT' then
    begin
...
procedure TfrmEstimate.SetaConsultaProduto;
var
...
begin
   ...
   Try
     ...
     begin
       ...
       begin
          bRetorno := False;
          Mens_MensInf('Terms definition is incomplete.') ;
          edtTotal.EditValue := sqlProcessTOTAL.AsFloat - sqlDados.FieldByName('Total').AsFloat;
          edtDays.SetFocus;
       end;
     end;
   Finally
...
begin
   ...
   Try
     ...
     if sqlDados.FieldByName('Total').AsFloat > 0 then
     begin
       if (sqlDados.FieldByName('Total').AsFloat  + edtTotal.EditValue ) > sqlProcessTOTAL.AsFloat then
       begin
          bRetorno := False;
          Mens_MensInf('Terms is incorrect.') ;
          edtTotal.EditValue := sqlProcessTOTAL.AsFloat - sqlDados.FieldByName('Total').AsFloat;
          edtDays.SetFocus;
       end;
     end;
