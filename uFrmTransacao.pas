...
uses
  ...
  Vcl.DBGrids, cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox, Vcl.Buttons;
...
type
  TfrmTransacao = class(TForm)
    ...
    cdsTransacaoALTERADO: TStringField;
    cdsTransacaoID_BANKING: TIntegerField;
    cdsTransacaoFORECAST: TSmallintField;
    cdsTransacaoID_USERBANK: TIntegerField;
    cdsTransacaoMODO: TStringField;
    cdsTransacaoID_COMPANY: TIntegerField;
    cdsTransacaoCONCILIADO: TIntegerField;
    cdsTransacaoNUM_PARCELA: TStringField;
    cdsTransacaoFOREIGN_KEY: TIntegerField;
    cdsTransacaoINTERNALCOD: TStringField;
    BitBtn1: TBitBtn;
...
uses uFrmVincularCentroCusto, uFrmAddTransacao;
...
begin
   ...
   if cxLookupComboBoxBank.EditValue <> -1 then
    ...
    begin
        ...
        if chkForecast.Checked = False then
          sqlGrafIE.SQL.Add('WHERE B.TIPO = ' + consTipoExpense + ' and FORECAST = 0  ')
        else  sqlGrafIE.SQL.Add('WHERE B.TIPO = ' + consTipoExpense + 'and FORECAST = 1  ');

        sqlGrafIE.SQL.Add( varPeriodo );
        sqlGrafIE.SQL.Add(' &WHERE1 ');
        sqlGrafIE.SQL.Add(' &WHERE2 ');
        sqlGrafIE.SQL.Add('AND B.INTERNALCOD IS NOT NULL  ');
        ...
        sqlGrafIE.SQL.Add('AND Upper(B.Categoria) = Upper(A.Categoria) ');
        ...
        sqlGrafIE.SQL.Add('WHERE C.TIPO = ' + consTipoIncome + ' and FORECAST = 0  ');
        sqlGrafIE.SQL.Add( varPeriodo );
        sqlGrafIE.SQL.Add(' &WHERE1 ');
        sqlGrafIE.SQL.Add(' &WHERE2 ');
        sqlGrafIE.SQL.Add('AND C.INTERNALCOD IS NOT NULL  ');
        ...
        sqlGrafIE.SQL.Add('AND Upper(C.Categoria) = Upper(A.Categoria) ');
        ...
        else  sqlGrafIE.SQL.Add('FROM TBTRANSACAO A WHERE  A.FORECAST = 1 ');
        sqlGrafIE.SQL.Add(')  ');
        ...
        sqlGrafIE.MacroByName( 'WHERE1' ).AsRaw := ' AND ID_COMPANY = ' + IntToStr( DBDados.varIDMAIN_COMPANY);

        if idx <> -1 then
          sqlGrafIE.MacroByName( 'WHERE2' ).AsRaw := ' AND ID_BANKING = ' + IntToStr(Integer(cxLookupComboBoxBank.EditValue));
    ...
    begin
       ...
       if chkForecast.Checked = False then
        sqlGrafIE.SQL.Add(' WHERE C.TIPO = ' + consTipoIncome + ' and FORECAST = 0')
       else   sqlGrafIE.SQL.Add(' WHERE C.TIPO = ' + consTipoIncome + ' and FORECAST = 1');
       sqlGrafIE.SQL.Add( varPeriodo );
       sqlGrafIE.SQL.Add(' &WHERE1 ');
       sqlGrafIE.SQL.Add(' &WHERE2 ');
       sqlGrafIE.SQL.Add('AND C.INTERNALCOD IS NOT NULL  ');
...
begin
   ...
   if cxLookupComboBoxBank.EditValue <> -1 then
    ...
    sqlGrafico1.SQL.Add('SELECT INTERNALCOD AS LEGENDA, SUM(VALOR * -1) AS VALOR ');
    sqlGrafico1.SQL.Add('FROM TBTRANSACAO ');
    if chkForecast.Checked = False then
      sqlGrafico1.SQL.Add('WHERE 1 = 1 and FORECAST = 0 ' )
    else   sqlGrafico1.SQL.Add('WHERE 1 = 1 and FORECAST = 1 ' );
    ...
    sqlGrafico1.SQL.Add('AND INTERNALCOD IS NOT NULL  ');
    sqlGrafico1.SQL.Add(' GROUP BY INTERNALCOD ');

    sqlGrafico1.MacroByName( 'WHERE1' ).AsRaw := ' AND ID_COMPANY = ' + IntToStr( DBDados.varIDMAIN_COMPANY);

    sqlGrafico1.MacroByName( 'WHERE2' ).AsRaw := ' AND INTERNALCOD = ' + QuotedStr(sqlRelat01.Fields.Fields[2].AsString);

   if idx <> -1 then
      sqlGrafico1.MacroByName( 'WHERE3' ).AsRaw := ' AND ID_USERBANK = ' + IntToStr(Integer(cxLookupComboBoxBank.EditValue));
...
begin
  ...
  begin
    ...
    begin
      ...
      varResultado      := varResultado + BuscaMes(NovaData, bMostraLabel,Alias,SomarL, TotalL, DBDados.varBanco);
...
begin
  if (DBDados.BuscaDados = False)  Then Exit;
...
begin
   ...
   begin
       ...
       sqlGraficoWeek.SQL.Add('SELECT INTERNALCOD,  DATEPART(week, DATATRANSACAO) as [WEEK], SUM(VALOR * -1) AS VALOR ');
       sqlGraficoWeek.SQL.Add(' FROM TBTRANSACAO ');
       if chkForecast.Checked = False then
         sqlGraficoWeek.SQL.Add(' WHERE 1 = 1 and FORECAST = 0 ')
       else   sqlGraficoWeek.SQL.Add(' WHERE 1 = 1 and FORECAST = 1 ');
       ...
       sqlGraficoWeek.SQL.Add(' AND INTERNALCOD IS NOT NULL ');
       sqlGraficoWeek.SQL.Add(' AND INTERNALCOD = :INTERNALCOD ');
       sqlGraficoWeek.SQL.Add(' GROUP BY INTERNALCOD,DATEPART(week, DATATRANSACAO) ');
       sqlGraficoWeek.SQL.Add(' ORDER BY DATEPART(week, DATATRANSACAO) ');

       sqlGraficoWeek.Params.ParamByName('INTERNALCOD').AsString := sqlRelat01.Fields.Fields[2].AsString;

       sqlGraficoWeek.MacroByName( 'WHERE1' ).AsRaw := ' AND ID_COMPANY = ' + IntToStr(DBDados.varIDMAIN_COMPANY);
       ...
       if idx <> -1 then
          sqlGraficoWeek.MacroByName( 'WHERE4' ).AsRaw := ' AND ID_BANKING = ' +IntToStr(Integer(cxLookupComboBoxBank.EditValue));
       ...
       sqlGraficoWeek.SQL.Add('SELECT INTERNALCOD,   strftime(''%w'', DATATRANSACAO) AS WEEK, IFNULL(Sum(CAST(VALOR * -1 AS DECIMAL)),0.00) AS VALOR ');
       sqlGraficoWeek.SQL.Add(' FROM TBTRANSACAO ');
       if chkForecast.Checked = False then
         sqlGraficoWeek.SQL.Add(' WHERE 1 = 1 and FORECAST = 0 ')
       else sqlGraficoWeek.SQL.Add(' WHERE 1 = 1 and FORECAST = 1 ');
       ...
       sqlGraficoWeek.SQL.Add(' AND INTERNALCOD IS NOT NULL ');
       sqlGraficoWeek.SQL.Add(' AND INTERNALCOD = :INTERNALCOD ');
       sqlGraficoWeek.SQL.Add(' GROUP BY INTERNALCOD, strftime(''%w'', DATATRANSACAO)  ');
...
begin
{
   ...
   begin
       ...
       if chkForecast.Checked = False then
         sqlDetalhe.SQL.Add(' Where INTERNALCOD = :INTERNALCOD and FORECAST = 0 and DataTransacao >= :MES_INI and  DataTransacao <= :MES_FIM ')
       else  sqlDetalhe.SQL.Add(' Where INTERNALCOD = :INTERNALCOD and FORECAST = 1 and DataTransacao >= :MES_INI and  DataTransacao <= :MES_FIM ');
       ...
       sqlDetalhe.Params.ParamByName('INTERNALCOD').AsString := sqlRelat01.Fields.Fields[2].AsString;
       ...
       sqlDetalhe.MacroByName( 'WHERE1' ).AsRaw := ' AND ID_COMPANY = ' + IntToStr( DBDados.varIDMAIN_COMPANY);

       if cxTipo.Text <> '' then
          sqlDetalhe.MacroByName( 'WHERE2' ).AsRaw := ' AND TIPO = ' + QuotedStr(cxTipo.Text);

       if idx <> -1 then
          sqlDetalhe.MacroByName( 'WHERE3' ).AsRaw := ' AND ID_BANKING = ' + IntToStr(Integer(cxLookupComboBoxBank.EditValue));
...
begin
  ...
  begin
      ...
      sqlGrafico1.SQL.Add(' AND DATATRANSACAO BETWEEN :MES_INI AND :MES_FIM  ');
      sqlGrafico1.SQL.Add(' AND INTERNALCOD IS NOT NULL  AND TIPO = ' + consTipoExpense);

      sqlGrafico1.SQL.Add('UNION ALL     ');

      sqlGrafico1.SQL.Add('SELECT CAST(INTERNALCOD AS VARCHAR(100)) AS LEGENDA, CAST(SUM(IFNULL(VALOR * -1,0.00)) AS NUMERIC(12,2)) AS VALOR   ');
      sqlGrafico1.SQL.Add('FROM TBTRANSACAO   ');
      if chkForecast.Checked = False then
        sqlGrafico1.SQL.Add('WHERE 1 = 1 and FORECAST = 0 ' )
      else  sqlGrafico1.SQL.Add('WHERE 1 = 1 and FORECAST = 1 ' );
  ...
  begin
      ...
      sqlGrafico1.SQL.Add('SELECT CAST(INTERNALCOD AS VARCHAR(100)) AS LEGENDA,  Sum(CAST(IFNULL(VALOR * -1,0.00) AS DECIMAL)) AS VALOR ');
      sqlGrafico1.SQL.Add('FROM TBTRANSACAO   ');
      if chkForecast.Checked = False then
        sqlGrafico1.SQL.Add('WHERE 1 = 1 and FORECAST = 0 ' )
      else   sqlGrafico1.SQL.Add('WHERE 1 = 1 and FORECAST = 1 ' );
...
begin
  ...
  else  sqlItem.SQL.Add('WHERE 1 = 1  and FORECAST = 1 ');
  ...
  sqlItem.SQL.Add(' AND INTERNALCOD IS NOT NULL  AND TIPO = ' + consTipoExpense);
  sqlItem.SQL.Add('Order by  CAST(DATATRANSACAO   AS DATE) desc   ');

   if DBDados.varUsuario  <> '' then
      sqlItem.MacroByName( 'WHERE1' ).AsRaw := ' AND ID_COMPANY = ' + IntToStr( DBDados.varIDMAIN_COMPANY);

  sqlItem.MacroByName( 'WHERE2' ).AsRaw := ' AND INTERNALCOD = ' + QuotedStr(sqlRelat01.Fields.Fields[2].AsString);

  if idx <> -1 then
      sqlItem.MacroByName( 'WHERE3' ).AsRaw := ' AND ID_USERBANK = ' +IntToStr(Integer(cxLookupComboBoxBank.EditValue));
...
begin
   if ((Page.ActivePage = TabDetalhe) or (Page.ActivePage = TabSpend)  or (Page.ActivePage = TabIE)) Then
     if ((DBDados.BuscaDados = False) or  (sqlRelat01.IsEmpty)) then
...
begin
   ...
   if DBDados.varBanco = '1' then
    sqlGraficoBarra.SQL.Add('SELECT INTERNALCOD,  MONTH(DATATRANSACAO) AS MES , YEAR(DATATRANSACAO) AS ANO, SUM(VALOR * -1)  AS VALOR ')
   else  sqlGraficoBarra.SQL.Add('SELECT INTERNALCOD,  strftime(''%m'', DATATRANSACAO) AS MES , strftime(''%Y'', DATATRANSACAO) AS ANO, CAST(SUM(VALOR * -1) AS DECIMAL)  AS VALOR ');

   sqlGraficoBarra.SQL.Add('FROM TBTRANSACAO ');
   if chkForecast.Checked = False then
    sqlGraficoBarra.SQL.Add('WHERE 1 = 1 and FORECAST = 0 ')
   else  sqlGraficoBarra.SQL.Add('WHERE 1 = 1 and FORECAST = 1 ');
   sqlGraficoBarra.SQL.Add(' AND DataTransacao between :MES_INI and :MES_FIM  ');

   sqlGraficoBarra.SQL.Add(' AND INTERNALCOD IS NOT NULL   ');
   sqlGraficoBarra.SQL.Add('    &WHERE1  ');
   sqlGraficoBarra.SQL.Add('    &WHERE2  ');
   sqlGraficoBarra.SQL.Add('    &WHERE3  ');
   if DBDados.varBanco = '1' then
   begin
     sqlGraficoBarra.SQL.Add('GROUP BY INTERNALCOD, MONTH(DATATRANSACAO) , YEAR(DATATRANSACAO) ');
     sqlGraficoBarra.SQL.Add(' ORDER BY MONTH(DATATRANSACAO) , YEAR(DATATRANSACAO) DESC ')
   end
    else
    begin
      sqlGraficoBarra.SQL.Add('GROUP BY INTERNALCOD, strftime(''%m'', DATATRANSACAO)  ,  strftime(''%Y'', DATATRANSACAO) ');
      sqlGraficoBarra.SQL.Add(' ORDER BY strftime(''%m'', DATATRANSACAO)  ,  strftime(''%Y'', DATATRANSACAO) DESC ');

    end;

   if DBDados.varUsuario  <> '' then
     sqlGraficoBarra.MacroByName( 'WHERE1' ).AsRaw := ' AND ID_COMPANY = ' + IntToStr(DBDados.varIDMAIN_COMPANY);

   sqlGraficoBarra.MacroByName( 'WHERE2' ).AsRaw := ' AND INTERNALCOD = ' + QuotedStr(sqlRelat01.Fields.Fields[2].AsString);

  if Idx <> -1 then
      sqlGraficoBarra.MacroByName( 'WHERE3' ).AsRaw := ' AND ID_USERBANK = ' + IntToStr(Integer(cxLookupComboBoxBank.EditValue));
 ...
 Try
     ...
     begin
        ...
        begin
           sqlAux.Close;
           sqlAux.SQL.Clear;
           sqlAux.SQL.Add('Update TBTransacao Set INTERNALCOD = :INTERNALCOD');
           sqlAux.SQL.Add(' Where ID = :ID ');
           //sqlAux.Params.ParamByName('INTERNALCOD').AsString := // frmVincularCentroCusto.cboCC.Text;
...
procedure TfrmTransacao.cxLookupComboBoxBankPropertiesChange(Sender: TObject);
begin
 if (DBDados.BuscaDados = False)  Then Exit;
 ...
 begin
   idx := cxLookupComboBoxBank.EditValue;
   DBDados.varID_Bank := DBDados.BuscaID_Bank(Integer(cxLookupComboBoxBank.EditValue));
...
begin
 ...
 Try
   ...
   if cxLookupComboBoxBank.EditValue <> -1 then
    ...
    if ((Balance = True) and (Forecast = False)) then
    begin
      ...
      sqlRelat01.SQL.Add('  LEFT OUTER JOIN TBEXPENSECATEGORY CC ON Upper(CC.INTERNALCOD) = Upper(T.INTERNALCOD)  AND Upper(T.CATEGORIA) = Upper(CC.CATEGORIA)  and CC.ID_COMPANY = T.ID_COMPANY and  CC.ID_COMPANY =' + IntToStr(DBDados.varIDMAIN_COMPANY) + ' and CC.ID_LANGUAGE = ' + IntToStr(DBDados.varID_Language));
      ...
      sqlRelat01.SQL.Add('  AND T.FORECAST IN (-1, 1) ');
    ...
    begin
      sqlRelat01.SQL.Add(' SELECT  ''R'' AS SOURCE, CC.GRUPO, T.INTERNALCOD [' + UpperCase(DBDados.RetornaMensagem('MEN_CENTROCUSTO')) + '], T.CATEGORIA ');
      sqlRelat01.SQL.Add( varMesesComLabel );
      sqlRelat01.SQL.Add( ',COALESCE(SUM(VALOR),0) AS Total ' );
      sqlRelat01.SQL.Add(' 	FROM TBTRANSACAO T   ');
      sqlRelat01.SQL.Add('  INNER JOIN INTERNALCOD CC ON Upper(CC.INTERNALCOD) = Upper(T.INTERNALCOD)  AND Upper(CC.CATEGORIA) = Upper(T.CATEGORIA)  and CC.ID_COMPANY = T.ID_COMPANY and  CC.ID_COMPANY =' + IntToStr(DBDados.varIDMAIN_COMPANY) + ' and CC.ID_LANGUAGE = ' + IntToStr(DBDados.varID_Language));
      sqlRelat01.SQL.Add(' 	WHERE  1 = 1  ');
      sqlRelat01.SQL.Add('  &WHERE1  ');
      sqlRelat01.SQL.Add('  &WHERE2  ');
      sqlRelat01.SQL.Add('  &WHERE3  ');
    ...
    begin
      sqlRelat01.SQL.Add(' SELECT  ''F'' AS SOURCE, CC.GRUPO, T.INTERNALCOD [' + UpperCase(DBDados.RetornaMensagem('MEN_CENTROCUSTO')) + '], T.CATEGORIA ');
      sqlRelat01.SQL.Add( varMesesComLabel );
      sqlRelat01.SQL.Add( ',COALESCE(SUM(VALOR),0) AS Total ' );
      sqlRelat01.SQL.Add(' 	FROM TBTRANSACAO T   ');
      sqlRelat01.SQL.Add('  LEFT OUTER JOIN TBEXPENSECATEGORY CC ON Upper(CC.INTERNALCOD) = Upper(T.INTERNALCOD)  AND Upper(T.CATEGORIA) = Upper(CC.CATEGORIA) and CC.ID_COMPANY = T.ID_COMPANY and   CC.ID_COMPANY =' + IntToStr(DBDados.varIDMAIN_COMPANY) + ' and CC.ID_LANGUAGE = ' + IntToStr(DBDados.varID_Language));
      ...
      sqlRelat01.SQL.Add(' 	&WHERE4 ');
      sqlRelat01.SQL.Add(' 	&WHERE5 ');
      sqlRelat01.SQL.Add(' GROUP BY   GRUPO, T.INTERNALCOD, T.INTERNALCOD ');
      sqlRelat01.SQL.Add(' ORDER BY CC.GRUPO ASC  ');
    ...
    sqlRelat01.MacroByName( 'WHERE1' ).AsRaw := ' AND T.ID_COMPANY = ' + IntToStr(DBDados.varIDMAIN_COMPANY);
   ...
   if idx <> -1 then
   begin
      sqlRelat01.MacroByName( 'WHERE3' ).AsRaw := ' AND T.ID_USERBANK = ' + IntToStr(Integer(cxLookupComboBoxBank.EditValue));
   end;
...
var
  varCentroCusto, varCategoria : String;
begin
 ...
 Try
   ...
   if cxLookupComboBoxBank.EditValue <> Null then
    ...
    sqlRelat01.SQL.Add('  ( ');
    varCentroCusto := DBDados.RetornaMensagem('MEN_CENTROCUSTO');
    varCategoria   := DBDados.RetornaMensagem('MEN_FIN_BENEF');

    if Balance then
    begin
      sqlRelat01.SQL.Add('   SELECT ''R'' AS SOURCE, 0 AS GRUPO, ''BALANCE'' [' + varCentroCusto + '], ''BALANCE'' [' + varCategoria +'] , ');
      sqlRelat01.SQL.Add( varMesesComLabelA );
      sqlRelat01.SQL.Add('FROM ');
      sqlRelat01.SQL.Add('( ');
      sqlRelat01.SQL.Add('	SELECT T.SubCategoria  ');
      sqlRelat01.SQL.Add('		 ,VALOR = CAST(SUM(ISNULL(T.VALOR,0.00)) AS NUMERIC(12,2)) ');
      sqlRelat01.SQL.Add('		 ,CONVERT(CHAR(4),T.DATATRANSACAO, 100) + CONVERT(CHAR(4), T.DATATRANSACAO, 120) AS MES  ');
      sqlRelat01.SQL.Add('	FROM TBTRANSACAO T');
      ...
      if chkForecast.Checked = False then
       sqlRelat01.SQL.Add('    AND T.FORECAST = 0 ')
      else   sqlRelat01.SQL.Add('    AND T.FORECAST = 1 ');
      sqlRelat01.SQL.Add(' 	AND T.DATATRANSACAO >= :MES_INIANTERIOR ');
      sqlRelat01.SQL.Add(' 	AND T.DATATRANSACAO <= :MES_FIMANTERIOR ');

      sqlRelat01.SQL.Add('	GROUP BY T.SubCategoria, T.DATATRANSACAO  ');
      sqlRelat01.SQL.Add('  ) A ');
      sqlRelat01.SQL.Add('  PIVOT ');
      sqlRelat01.SQL.Add('  ( ');
      sqlRelat01.SQL.Add('   SUM(VALOR)  ');
      sqlRelat01.SQL.Add('  FOR Mes IN ( ' + varMesesSemLabelA + ')');
      sqlRelat01.SQL.Add(' ) AS PIVOTTABLE ');
    ...
    if Forecast = False then
    begin
      sqlRelat01.SQL.Add(' SELECT  ''R'' AS SOURCE, GRUPO, SubCategoria [' + varCentroCusto + '],  CATEGORIA [' + varCategoria +'], ');
      sqlRelat01.SQL.Add( varMesesComLabel );
      sqlRelat01.SQL.Add(' FROM  ');
      sqlRelat01.SQL.Add(' (  ');
      sqlRelat01.SQL.Add(' 	SELECT T.SubCategoria , CC.GRUPO , T.CATEGORIA ');
      sqlRelat01.SQL.Add(' 		 ,VALOR = CAST(SUM(ISNULL(T.VALOR,0.00)) AS NUMERIC(12,2))   ');
      sqlRelat01.SQL.Add(' 		 ,CONVERT(CHAR(4),T.DATATRANSACAO, 100) + CONVERT(CHAR(4), T.DATATRANSACAO, 120) AS MES  ');
      sqlRelat01.SQL.Add(' 	FROM TBTRANSACAO T    ');
      sqlRelat01.SQL.Add(' 	LEFT OUTER JOIN TBEXPENSECATEGORY CC ON CC.INTERNALCOD = T.INTERNALCOD  AND T.TIPO = CC.TIPO ');
      sqlRelat01.SQL.Add(' 	WHERE  1 = 1  ');
      sqlRelat01.SQL.Add(' 	AND  CC.ID_LANGUAGE = :ID_LANGUAGE  AND CC.ID_COMPANY = :ID_COMPANY ');
      ...
      sqlRelat01.SQL.Add(' 	AND T.DATATRANSACAO >= :MES_INI ');
      sqlRelat01.SQL.Add(' 	AND T.DATATRANSACAO <= :MES_FIM    ');

      sqlRelat01.SQL.Add(' 	GROUP BY T.SubCategoria, T.DATATRANSACAO , CC.GRUPO,  T.CATEGORIA  ');
    ...
    begin
        sqlRelat01.SQL.Add(' SELECT  ''F'' AS SOURCE, GRUPO, SubCategoria  [' + varCentroCusto + '],  CATEGORIA [' + varCategoria +'], ');
        sqlRelat01.SQL.Add( varMesesComLabel );
        sqlRelat01.SQL.Add(' FROM  ');
        sqlRelat01.SQL.Add(' (  ');
        sqlRelat01.SQL.Add(' 	SELECT T.SubCategoria , CC.GRUPO , T.CATEGORIA ');
    ...
    sqlRelat01.SQL.Add(' SELECT *,  0 as TOTAL FROM DADOS WHERE  [' + varCentroCusto + '] = ''BALANCE''');
    sqlRelat01.SQL.Add(' UNION ALL ');
    sqlRelat01.SQL.Add(' SELECT *, ' + varMesesSemLabelT  + ' as TOTAL FROM DADOS WHERE  [' + varCentroCusto + '] <> ''BALANCE''');

    sqlRelat01.SQL.Add('  ORDER BY GRUPO, [' + varCentroCusto + ']  ');
    ...
    sqlRelat01.Params.ParamByName( 'ID_COMPANY' ).AsInteger  :=  DBDados.varIDMAIN_COMPANY;
    sqlRelat01.Params.ParamByName( 'ID_LANGUAGE' ).AsInteger  :=  1; //Dados.varID_Language;

    lblPeriodo.Caption := 'Period: ' + DateToStr(IncMonth(IncMonth(System.DateUtils.StartOfTheMonth(dtDateTrans), +1), -13)) + ' - ' +
              DateToStr(IncMonth(IncMonth(System.DateUtils.EndOfTheMonth(dtDateTrans), -12), 12));

    sqlRelat01.MacroByName( 'WHERE1' ).AsRaw := ' AND ' + 'T.' + DBDados.varReturnCompanies;

    if cxTipo.Text <> '' then
      sqlRelat01.MacroByName( 'WHERE2' ).AsRaw := ' AND T.TIPO = ' + QuotedStr(cxTipo.Text);

   if idx <> -1 then
      sqlRelat01.MacroByName( 'WHERE3' ).AsRaw := ' AND T.ID_BANKING = ' + IntToStr(Integer(cxLookupComboBoxBank.EditValue));
...
begin
   ...
   try
    ...
    ABand.Index := 1;
    ABand.HeaderAlignmentHorz := taCenter;
    ABand.Caption :=  DBDados.RetornaMensagem('MEN_VALUE');
...
procedure TfrmTransacao.BitBtn1Click(Sender: TObject);
begin
  if DBDados.varLogado = False  then Exit;
   ...
   begin
       ...
       begin
            ...
            begin
              ...
              MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0]  + '),0)   AS ' + varMesAnterior[0] + ',' ;
            end
            else
            ...
            begin
              ...
              Case Mes Of
                   1   : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0)  AS ' + varMesAnterior[1]  + ',' ;
                   2   : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) AS ' + varMesAnterior[2]  + ',' ;
                   3   : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) + IsNull(Sum( ' + varMesAnterior[2] + '),0) AS ' + varMesAnterior[3]  + ',' ;
                   4   : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) + IsNull( Sum( ' + varMesAnterior[2] + '),0) + IsNull(Sum ( ' + varMesAnterior[3] + '),0) AS ' + varMesAnterior[4]  + ',' ;
                   5   : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) + IsNull( Sum( ' + varMesAnterior[2] + '),0) + IsNull(Sum ( ' + varMesAnterior[3] + '),0) + IsNull(Sum ( ' + varMesAnterior[4] + '),0) AS ' + varMesAnterior[5]  + ',' ;
                   6   : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) + IsNull( Sum( ' + varMesAnterior[2] + '),0) + IsNull(Sum ( ' + varMesAnterior[3] + '),0) + IsNull(Sum ( ' + varMesAnterior[4] + '),0) + IsNull(Sum ( ' + varMesAnterior[5] + '),0) AS ' + varMesAnterior[6]  + ',' ;
                   7   : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) + IsNull( Sum( ' + varMesAnterior[2] + '),0) + IsNull(Sum ( ' + varMesAnterior[3] + '),0) + IsNull(Sum ( ' + varMesAnterior[4] + '),0) + IsNull(Sum ( ' + varMesAnterior[5] + '),0) + IsNull(Sum ( ' + varMesAnterior[6] + '),0) AS ' + varMesAnterior[7]  + ',' ;
                   8   : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) + IsNull( Sum( ' + varMesAnterior[2] + '),0) + IsNull(Sum ( ' + varMesAnterior[3] + '),0) + IsNull(Sum ( ' + varMesAnterior[4] + '),0) + IsNull(Sum ( ' + varMesAnterior[5] + '),0) + IsNull(Sum ( ' + varMesAnterior[6] + '),0) + IsNull(Sum ( ' + varMesAnterior[7] + '),0) AS ' + varMesAnterior[8]  + ',' ;
                   9   : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) + IsNull( Sum( ' + varMesAnterior[2] + '),0) + IsNull(Sum ( ' + varMesAnterior[3] + '),0) + IsNull(Sum ( ' + varMesAnterior[4] + '),0) + IsNull(Sum ( ' + varMesAnterior[5] + '),0) + IsNull(Sum ( ' + varMesAnterior[6] + '),0) + IsNull(Sum ( ' + varMesAnterior[7] + '),0) + IsNull(Sum ( ' + varMesAnterior[8] + '),0) AS ' + varMesAnterior[9]  + ',' ;
                   10  : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) + IsNull( Sum( ' + varMesAnterior[2] + '),0) + IsNull(Sum ( ' + varMesAnterior[3] + '),0) + IsNull(Sum ( ' + varMesAnterior[4] + '),0) + IsNull(Sum ( ' + varMesAnterior[5] + '),0) + IsNull(Sum ( ' + varMesAnterior[6] + '),0) + IsNull(Sum ( ' + varMesAnterior[7] + '),0) + IsNull(Sum ( ' + varMesAnterior[8] + '),0) + IsNull(Sum ( ' + varMesAnterior[9] + '),0) AS ' + varMesAnterior[10]  + ',' ;
                   11  : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) + IsNull( Sum( ' + varMesAnterior[2] + '),0) + IsNull(Sum ( ' + varMesAnterior[3] + '),0) + IsNull(Sum ( ' + varMesAnterior[4] + '),0) + IsNull(Sum ( ' + varMesAnterior[5] + '),0) + IsNull(Sum ( ' + varMesAnterior[6] + '),0) + IsNull(Sum ( ' + varMesAnterior[7] + '),0) + IsNull(Sum ( ' + varMesAnterior[8] + '),0) + IsNull(Sum ( ' + varMesAnterior[9] + '),0) + IsNull(Sum ( ' + varMesAnterior[10] + '),0) AS ' + varMesAnterior[11]   + ',' ;
                   12  : MesFiscalAnterior := 'IsNull(Sum(' +  varMesAtual[0] + '),0) + IsNull(Sum(' + varMesAnterior[0] + '),0) + IsNull(Sum( ' + varMesAnterior[1]  + '),0) + IsNull( Sum( ' + varMesAnterior[2] + '),0) + IsNull(Sum ( ' + varMesAnterior[3] + '),0) + IsNull(Sum ( ' + varMesAnterior[4] + '),0) + IsNull(Sum ( ' + varMesAnterior[5] + '),0) + IsNull(Sum ( ' + varMesAnterior[6] + '),0) + IsNull(Sum ( ' + varMesAnterior[7] + '),0) + IsNull(Sum ( ' + varMesAnterior[8] + '),0) + IsNull(Sum ( ' + varMesAnterior[9] + '),0) + IsNull(Sum ( ' + varMesAnterior[10] + '),0) + IsNull(Sum ( ' + varMesAnterior[11] + '),0) AS ' + varMesAnterior[12]   + ',' ;
