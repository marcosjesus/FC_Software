...
begin
  ...
  begin
     Mens_MensInf('Unit Price field is required.') ;
  ...
  begin
     Mens_MensInf('Subtotal field is required.') ;
  ...
  edtMerchandise.Value := edtMerchandise.Value + Item.amount;
...
begin
   ...
   Try
     ...
     sqlDados.Params.ParamByName('ID_PROCESS').AsInteger := sqlGridID.AsInteger;
     sqlDados.Open;
     if sqlDados.FieldByName('Total').AsFloat > 0 then
     begin
       if (sqlDados.FieldByName('Total').AsFloat  + edtTotal.EditValue ) > sqlGridTOTAL.AsFloat then
       begin
          bRetorno := False;
          Mens_MensInf('Terms is incorrect.') ;
          edtTotal.EditValue := sqlGridTOTAL.AsFloat - sqlDados.FieldByName('Total').AsFloat;
          edtDays.SetFocus;
       end;
     end;
   Finally
...
begin
  ...
  sqlTerms.Params.ParamByName('TABLENAME').AsString   := SUP_INVOICE_ITEM;
  sqlTerms.Open;
end;
...
begin
   ...
   edtTotalTerms.Value := edtTotal.Value;
end;
